...
import {
  didDartsPlayerWin,
  dartsMatchHref,
  formatDartsHistoryDate,
  formatDartsHistoryPlayers,
  formatDartsHistoryScore,
} from "@/lib/darts/history";
import type { DartsHistoryItem } from "@/types/darts-match";

export function DartsHistoryList({
  items,
  playerUserId,
}: {
  items: DartsHistoryItem[];
  ...
  return (
    <ul className="divide-y divide-white/8 overflow-hidden rounded-3xl border border-white/8 bg-[#141814]">
      {items.map((item) => {
        const venueName = item.venueName?.trim() || "Home game";
        const won = playerUserId ? didDartsPlayerWin(item, playerUserId) : null;
          ...
          <li key={item.id} className="px-5 py-4 sm:px-6">
            <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  {formatDartsHistoryDate(item.startsAt)}
                </time>
              </div>
            ...
            {item.players.length > 0 ? (
              <p className="mt-1 text-sm text-zinc-400">
                {formatDartsHistoryPlayers(item.players)} · 501
              </p>
            ) : (
              <p className="mt-1 text-sm text-zinc-400">501 double-out</p>
            )}
            <Link
              href={dartsMatchHref(item.id)}
