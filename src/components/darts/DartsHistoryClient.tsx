...
import {
  ...
  DartsApiError,
  listPlayerDartsHistory,
} from "@/lib/darts/api-match";
import { dartsPlayerHistoryPath } from "@/lib/darts/history";
...
export function DartsHistoryClient() {
  ...
  const [items, setItems] = useState<DartsHistoryItem[] | null>(null);
  ...
  const load = useCallback(async (userId: string) => {
    ...
    setError(null);
    try {
      setItems(await listPlayerDartsHistory(userId));
    } catch (err) {
      setItems(null);
  ...
  async function handleShare() {
    ...
    const url = `${window.location.origin}${dartsPlayerHistoryPath(playerUserId)}`;
    try {
      ...
      {waitingForAuth || loadingList ? (
        ...
        <div className="space-y-4 rounded-3xl border border-white/8 bg-[#141814] px-5 py-6">
          <p className="text-sm leading-relaxed text-zinc-400">
            Locked games are listed by account. Guest names are not a history
        ...
        <p className="text-sm text-zinc-500">
          No locked games yet. Finish a live scorecard to write the first
          result.
        </p>
      ) : items ? (
