...
export function PadelHistoryClient() {
  ...
  const [items, setItems] = useState<PadelHistoryItem[] | null>(null);
  ...
  const load = useCallback(async (userId: string) => {
    ...
    setError(null);
    try {
      setItems(await listPlayerHistory(userId));
    } catch (err) {
      setItems(null);
  ...
  async function handleShare() {
    ...
    const url = `${window.location.origin}${playerHistoryPath(playerUserId)}`;
    try {
    ...
    <div className="space-y-6">
      {waitingForAuth || loadingList ? (
        ...
        <div className="space-y-4 rounded-3xl border border-white/8 bg-[#141814] px-5 py-6">
          <p className="text-sm leading-relaxed text-zinc-400">
            Locked results are listed by account. Guest names are not a
            history list — sign in, or open a shareable player link.
          </p>
          <button
            type="button"
            onClick={() => signIn("/padel/history")}
      ...
      {!isAuthenticated && sharedId ? (
        <p className="text-sm text-zinc-500">
          ...
          to see your own locked matches.
        </p>
      ) : null}
    </div>
  );
