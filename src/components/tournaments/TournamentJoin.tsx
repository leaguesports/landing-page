...
function sendToLogin(token: string) {
  const returnTo =
    typeof window === "undefined"
      ? tournamentJoinHref(token)
      : relativeAuthReturnTo() || tournamentJoinHref(token);
  window.location.href = getLoginPageHref(returnTo);
}

export function TournamentJoin({ token, teams }: TournamentJoinProps) {
  ...
  function onJoin() {
    ...
    startTransition(() => {
      void joinTournament({ token, teamId }).then((result) => {
        if (!result.ok) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-20">
      ...
      <p className="mt-4 max-w-md text-sm leading-relaxed text-zinc-400">
        Register a same-sport team you own or captain. Accepted entries fill the
        bracket — no byes.
      </p>

      {staffTeams.length === 0 ? (
        <p className="mt-6 text-sm text-zinc-400">
          You need an owner or captain role on a team to register.
        </p>
      ) : (
        <div className="mt-6 space-y-3">
          <label
            htmlFor="join-tournament-team"
            className="block text-xs font-medium text-zinc-400"
          >
            Register
          </label>
          <select
            id="join-tournament-team"
            value={teamId}
      ...
      >
        {isAuthenticated ? "Register team" : "Sign in to join"}
      </button>
      {error ? (
