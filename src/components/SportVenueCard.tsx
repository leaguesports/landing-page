...
export function SportVenueCard({
  ...
  return (
    <Link
      href={href}
      className={`group relative flex flex-col rounded-2xl overflow-hidden border border-white/10 bg-white/5 transition-all duration-300 hover:-translate-y-0.5 ${styles.card}`}
    >
      ...
      <div className="relative flex flex-1 flex-col p-5">
        ...
        {area && (
          <p className="mt-1 flex items-center gap-1.5 text-sm text-white/70">
            <MapPin className={`h-4 w-4 shrink-0 ${styles.mapPin}`} strokeWidth={2} />
            {area}
          </p>
        )}
        <span className={`mt-4 inline-flex items-center gap-1 text-sm font-semibold transition-colors ${styles.cta}`}>
          View venue
