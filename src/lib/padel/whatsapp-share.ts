...
export function padelMatchAbsoluteUrl(matchId: string, origin: string): string {
  return `${origin.replace(/\/$/, "")}/padel/${matchId}`;
}

function whatsAppTeamLabel(label: string): string {
...
export function padelShareText(input: PadelShareTextInput): string {
  ...
  const teams = `${whatsAppTeamLabel(input.teamALabel)} vs ${whatsAppTeamLabel(input.teamBLabel)}`;
  if (venue) {
...
export function buildPadelWhatsAppShare(
  match: Pick<PadelMatch, "id" | "pairings" | "venue">,
  origin: string,
): PadelWhatsAppShare {
  const text = padelShareText({
    matchId: match.id,
    teamALabel: getTeamLabel(match, "A"),
    teamBLabel: getTeamLabel(match, "B"),
